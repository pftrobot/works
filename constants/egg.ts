...
export const getFieldIdByPath = (pathname: string): string => {
  if (pathname === '/') return FIELD_IDS.HOME
  if (pathname.startsWith('/about')) return FIELD_IDS.ABOUT
  if (pathname.startsWith('/case')) return FIELD_IDS.CASE
  if (pathname.startsWith('/contact')) return FIELD_IDS.CONTACT
  if (pathname.startsWith('/medal')) return FIELD_IDS.MEDAL
  return FIELD_IDS.DEFAULT
}
